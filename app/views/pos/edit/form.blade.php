		{{ Form::model( $po , array('method' => 'PATCH' , 'route' => array('pos.update', $po->getPoIdentifier()), 'id' => 'poForm') ) }}
		<div class="row">
			<div class="col-lg-7">
				<div class="panel panel-default {{ set_panel_error('name', $errors) }}">
					...
					<div class="panel-body">
						...
						{{ Form::text('name', $po->name, array('class' => 'form-control', 'placeholder' => 'e.g., New Office Desk')) }}
					</div>
				</div>
				<div class="panel panel-default {{ set_panel_error('vendor', $errors) }}">
					...
					<div class="panel-body">
						...
						{{ Form::textarea('vendor', $po->vendor, array('rows' => 10, 'style' => 'width:100%')) }}
					</div>
				</div>
			</div>
			<div class="col-lg-5">
						...
						<div class="row">
							<div class="col-lg-6 col-sm-6">
								@foreach( $divisions as $division )
									<div style="margin-bottom:0px">{{ Form::checkbox('po[divisions][]', $division->id, in_array($division->id, $selectedDivisions) ) }} <span style="position:relative;top:-2px">{{ $division->name }}</span></div>
								@endforeach
							</div>
				...
				<div class="panel panel-default {{ $locationErrorsExist ? 'panel-danger' : '' }}">
					...
					<div class="panel-body" style="border-bottom:1px solid #DDDDDD">
						...
						@foreach( $locations as $location )
							<div>{{ Form::checkbox('po[locations][]', $location->id, in_array($location->id, $selectedLocations)  ) }} {{ $location->address }}</div>
						@endforeach
					</div>
					  ...
					  <div class="panel panel-default">
					    ...
					    <div id="collapseTwo" class="panel-collapse collapse in">
					      <div class="panel-body">
							...
							{{ Form::textarea('ship_to', $po->ship_to, array('rows' => 5, 'style' => 'width:100%')) }}
					      </div>
					    </div>
					  </div>
					</div>
									...
									@if( ! Session::get('_old_input.po.items') )
										@for ($i = 0, $c = count( $po->items ); $i < $c; $i++)
											<tr class="new" row="po[items][{{ $i }}][row_id]">
												<td class="#">{{ $i }}</td>
												<td class="#">{{ Form::text('po[items]['.$i.'][description]' , $po->items[$i]['description'] , array('class' => 'form-control')) }}</td>
												<td class="#">{{ Form::text('po[items]['.$i.'][due_date]'    , $po->items[$i]['due_date']    , array('class' => 'form-control')) }}</td>
												<td class="#">{{ Form::text('po[items]['.$i.'][qty]'	     , $po->items[$i]['qty']         , array('class' => 'form-control text-centered')) }}</td>
												<td class="#">{{ Form::text('po[items]['.$i.'][um]'		     , $po->items[$i]['um']          , array('class' => 'form-control text-centered')) }}</td>
												<td class="#">{{ Form::text('po[items]['.$i.'][uc]'		     , $po->items[$i]['uc']          , array('class' => 'form-control text-centered')) }}</td>
												<td class="#">{{ Form::text('po[items]['.$i.'][uc_um]'	     , $po->items[$i]['uc_um']       , array('class' => 'form-control text-centered')) }}</td>
												<td class="#">{{ Form::text('po[items]['.$i.'][tax]'	     , $po->items[$i]['tax'] , array('class' => 'form-control text-centered')) }}</td>
												<td class="#">{{ Form::text('po[items]['.$i.'][total]'	     , $po->items[$i]['total'], array('class' => 'form-control text-centered')) }}</td>
		...
		<div class="row">
			<div class="col-lg-4">
				<div class="panel panel-info">
					<div class="panel-heading">
						<h3 class="panel-title">Save as a draft.</h3>
					</div>
					<div class="panel-body">
						<p>By clicking "save as draft" your purchase order will not be submitted to accounting.</p>
						{{ Form::submit('Save as Draft', array('class' => 'btn btn-info btn-lg btn-block', 'id' => 'submitFormDraft')) }}
						{{ Form::hidden('draft', $po->draft ) }}
					</div>
				</div>
			</div>
			<div class="col-lg-4">
